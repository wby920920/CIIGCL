...
from chem.util import myDataset
...
def read_data(name):
    if name == 'abide1':
        filename = filename = 'Data/ABIDE1/label.mat'
        labels = io.loadmat(filename)['labels']
        labels = torch.transpose(torch.from_numpy(labels), 0, 1)
        # node_data: timeseries
        filename = 'Data/ABIDE1/timeseries.mat'
        timeseries = io.loadmat(filename)['timeseries']
        timeseries = np.transpose(timeseries, axes=(0, 2, 1))
        timeseries = torch.from_numpy(timeseries)
        # node_data: subjects_adj
        filename = 'Data/ABIDE1/corrf.mat'
        subjects_adj = np.array(io.loadmat(filename)['corrsf'])
        adj_data = torch.from_numpy(subjects_adj)
        adj_data = torch.tensor(adj_data, dtype=torch.float32)
        filename = 'Data/ABIDE1/partials.mat'
        sparse_anchors = np.array(io.loadmat(filename)['partials_Ledoit'])
        dataset = myDataset(adj_data, labels, sparse_anchors)
        data_list = []
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='abide1',
