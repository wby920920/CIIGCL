...
def symmetric_normalize_adj(adj):
    ...
    degrees_sqrt_inv = torch.pow(degrees, -0.5)
    degrees_sqrt_inv[torch.isinf(degrees_sqrt_inv)] = 0.0
    adj_normalized = torch.matmul(torch.matmul(torch.diag(degrees_sqrt_inv), adj), torch.diag(degrees_sqrt_inv))
    return adj_normalized
class myDataset(Dataset):
    def __init__(self, node_data, labels, subjects_adj):
        super(myDataset, self).__init__()
        mean = torch.mean(node_data, dim=(0, 1), keepdim=True)
        std = torch.std(node_data, dim=(0, 1), keepdim=True)
        self.node_data = (node_data - mean) / std
        self.labels = labels
    ...
    def __getitem__(self, idx):
        x = self.node_data[idx]  # 获取节点数据
        y = self.labels[idx]  # 获取标签
        adj =self.subjects_adj[idx]
        threshold = np.percentile(adj, 80)  # 保留10%的连接
        adj[adj < threshold] = 0
        np.fill_diagonal(adj, 1)
        adj = torch.from_numpy(adj)
        #邻接矩阵对称归一化
        adj = symmetric_normalize_adj(adj)
        edge_index = torch.nonzero(adj, as_tuple=False).t()
        edge_attr = adj[edge_index[0], edge_index[1]].clone().detach().float()
        data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=y.long())
        return data
